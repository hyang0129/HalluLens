...
def test_hallucination_evaluator_supports_knn_metric_from_embeddings():
    from activation_research.metric_evaluator import HallucinationEvaluator

    class DummyLoader:
        dataset = None

    # Baseline (train) embeddings clustered near origin
    train_records = [
    ...
    # Test embeddings: two near origin (ID) + two far away (OOD)
    test_embeddings = [
    ...
    # Avoid needing a model/dataloader by injecting the cached baseline.
