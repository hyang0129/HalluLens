...
def test_confirmation_experiments_are_isolated_to_two_missing_seeds():
    payloads = []
    ...
    assert all(payload["training_seeds"] == [1, 2] for payload in payloads)
    assert all(payload["split_seeds"] == [1, 2] for payload in payloads)
...
def test_builder_adds_exact_ten_matched_high_priority_cells(tmp_path):
    dispatch_root = tmp_path / "dispatch"
    runs_root = tmp_path / "runs"
    _write_baselines(runs_root)

    assert build(dispatch_root, project_root=_ROOT, runs_root=runs_root) == 10
    ...
    assert {cell["seed"] for cell in cells} == {1, 2}
...
def test_builder_refuses_when_a_matched_baseline_is_missing(tmp_path):
    dispatch_root = tmp_path / "dispatch"
    runs_root = tmp_path / "runs"
    _write_baselines(runs_root, omit=("nq_memmap", 2))

    with pytest.raises(FileNotFoundError, match="seed_2"):
