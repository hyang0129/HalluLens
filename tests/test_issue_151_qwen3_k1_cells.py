...
def test_qwen_tokenwise_v1_uses_full_36_layer_depth_trajectory():
    method = load_method_config(_TOKENWISE, project_root=str(_ROOT))
...
def test_qwen_experiments_cover_five_paired_datasets_without_mmlu():
    expected_methods = [_TOKENWISE, _ACTVIT_K1, _ACTVIT_EVAL]
    payloads = []
    for dataset, slug in _DATASETS.items():
        path = (
            _ROOT
        ...
        assert payload["methods"] == expected_methods
...
def test_builder_adds_75_cells_and_links_all_qwen_checkpoints(tmp_path):
    ...
    by_method = {
        ...
        for method in (_TOKENWISE, _ACTVIT_K1, _ACTVIT_EVAL)
    }
    assert all(len(method_cells) == 25 for method_cells in by_method.values())
    assert all(
    ...
    assert all(
        ...
        for cell in by_method[_TOKENWISE]
    )
