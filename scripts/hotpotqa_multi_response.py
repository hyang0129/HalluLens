#!/usr/bin/env python3
"""
hotpotqa_multi_response.py

Multi-response HotpotQA inference for hallucination correlation measurement.
Identical structure to nq_multi_response.py but uses HotpotQA data.
...
Outputs under shared/hotpotqa_hallu_cor/:
    generation_r0.jsonl ... generation_r4.jsonl
    eval_results.json
    per_question_phase1.jsonl
    per_question_phase2.jsonl (if temperature adjustment triggered)
"""
...
MAX_TOKENS         = 64
OUTPUT_DIR         = "shared/hotpotqa_hallu_cor"
PORT               = 8000
HOST               = "0.0.0.0"
DISAGREEMENT_TARGET = 0.20
...
def hotpotqa_is_correct(generation: str, answer: str) -> bool:
...
def run_inference_round(questions, response_idx, temperature, out_path, model, port, max_tokens):
    with filelock.FileLock(str(out_path) + ".lock"):
        ...
        logger.info(f"[r{response_idx}] Running {len(todo)}/{len(questions)} questions (temperature={temperature})")

        client = openai.OpenAI(base_url=f"http://localhost:{port}/v1", api_key="NOT_A_REAL_KEY", timeout=90.0)
        ...
        with open(out_path, "a") as f:
            for i, item in enumerate(todo):
                ...
                rec = {
                    ...
                    "generation": generation, "response_idx": response_idx,
...
def evaluate_and_correlate(questions, output_dir, n_responses):
    ...
    for q, info in q_map.items():
        answer = info["answer"]
        gens = info["gens"]
        labels = [int(not hotpotqa_is_correct(gens.get(r, ""), answer)) for r in range(n_responses)]
        ...
        per_question.append({
            "question": q, "answer": answer, "labels": labels,
            "majority": majority, "disagree_frac": disagree_frac,
        })
...
def print_summary(metrics, temperature):
    logger.info("=" * 70)
    logger.info("HALLUCINATION CORRELATION SUMMARY — HotpotQA")
    logger.info("=" * 70)
...
def main():
    ...
    from tasks.llmsknow.hotpotqa import load_hotpotqa
    questions = load_hotpotqa(split="validation", n_samples=args.N)
    ...
    if not args.no_server:
        if lm.check_server_health(f"http://localhost:{args.port}"):
            ...
            server_manager = lm.ServerManager(
                model=MODEL, host=HOST, port=args.port,
                logger_type="zarr", activations_path=activations_path,
    ...
    try:
        ...
        for r in range(N_RESPONSES):
            ...
            run_inference_round(
                questions=questions, response_idx=r, temperature=temp,
                out_path=output_dir / f"generation_r{r}.jsonl",
                model=MODEL, port=args.port, max_tokens=MAX_TOKENS,
            )
        ...
        print_summary(metrics, stochastic_temperature)

        results = {
            "phase": 1, "stochastic_temperature": stochastic_temperature,
        ...
        if metrics.get("mean_disagree", 1.0) < DISAGREEMENT_TARGET:
            ...
            logger.warning(f"Mean disagreement {metrics['mean_disagree']:.3f} < {DISAGREEMENT_TARGET}. Re-running at T={TEMPERATURE_HIGH}...")
            ...
            for r in range(1, N_RESPONSES):
                ...
                run_inference_round(
                    questions=questions, response_idx=r, temperature=stochastic_temperature,
                    out_path=out_path, model=MODEL, port=args.port, max_tokens=MAX_TOKENS,
                )

            metrics2 = evaluate_and_correlate(questions, output_dir, N_RESPONSES)
            print_summary(metrics2, stochastic_temperature)

            results["phase2_stochastic_temperature"] = stochastic_temperature
            results.update({f"phase2_{k}": v for k, v in metrics2.items() if k != "per_question"})
            ...
            logger.success(f"Disagreement {metrics['mean_disagree']:.3f} >= {DISAGREEMENT_TARGET} — no temperature adjustment needed.")
