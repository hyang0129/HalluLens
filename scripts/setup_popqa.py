#!/usr/bin/env python3
"""
Standalone PopQA setup: inference + evaluation.

Reproduces the PopQA notebook pipeline from the command line.

Usage:
  python scripts/setup_popqa.py --model meta-llama/Llama-3.1-8B-Instruct [--split test] [--N 100] [--batch-size 8]
"""
...
def main():
    parser = argparse.ArgumentParser(description="PopQA: inference + evaluation pipeline")

    parser.add_argument("--model", type=str, required=True, help="Model name or path")
    parser.add_argument("--split", type=str, default="test", choices=["test", "train"],
                        help="Dataset partition (default: test = ~2,853 questions)")
    ...
    task_dir = Path(args.output_dir) / "popqa" / model_name
    ...
    print("=" * 60)
    print("PopQA Setup Pipeline")
    print("=" * 60)
    print(f"Model          : {args.model}")
    print(f"Split          : {args.split}  (N={args.N or 'all'}, seed={args.split_seed})")
    ...
    from tasks.llmsknow.popqa import run_step
