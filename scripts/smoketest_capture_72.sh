#!/bin/bash
# GPU smoketest for the #72 capture rewrite — sciq × Llama-3.1-8B-Instruct, N=50.
...
LOGDIR=$REPO/logs
mkdir -p "$LOGDIR"
LOG="$LOGDIR/issue72_smoketest_sciq_llama_$(date +%Y%m%d_%H%M%S).log"

OUT_DIR=$REPO/shared/icr_capture/sciq_Llama-3.1-8B-Instruct_smoketest
...
out_dir = Path("/mnt/home/hyang1/LLM_research/HalluLens/shared/icr_capture/sciq_Llama-3.1-8B-Instruct_smoketest")
