...
def check_dependencies(task, step):
    ...
    if task == "precisewikiqa":
        ...
        # PopQA auto-downloads from HuggingFace, no local data files needed
        output_dir = project_root / "output"
        output_dir.mkdir(parents=True, exist_ok=True)
...
def run_task_step(step, task, model, **kwargs):
    ...
    try:
        if task == "precisewikiqa":
            ...
            if step == "generate":
                logger.info("MMLU doesn't have a generate step — loaded directly from HuggingFace")
                return None
            from tasks.llmsknow.mmlu import run_step as _run
            _run(
                step=step,
                model=model,
                output_dir=kwargs.get("output_dir", "output"),
            ...
            if step == "generate":
                logger.info("PopQA doesn't have a generate step — loaded directly from HuggingFace")
                return None
            from tasks.llmsknow.popqa import run_step as _run
            _run(
                step=step,
                model=model,
                output_dir=kwargs.get("output_dir", "output"),
            ...
            if step == "generate":
                logger.info("SciQ doesn't have a generate step — loaded directly from HuggingFace")
                return None
            from tasks.llmsknow.sciq import run_step as _run
            _run(
                step=step,
                model=model,
                output_dir=kwargs.get("output_dir", "output"),
...
def main():
    ...
    parser.add_argument("--task", required=True, choices=["precisewikiqa", "longwiki", "mixedentities", "triviaqa", "naturalquestions", "truthfulqa", "hotpotqa", "movies", "mmlu", "popqa", "searchqa", "sciq"],
