"""
generate_manifest_attn_pool.py — populate pending/ with one JSON cell per
(experiment_config × method × seed) for the Issue #120 attention-pooling sweep.

Usage:
    python scripts/dispatch/generate_manifest_attn_pool.py \
        --dispatch-root shared/issue_120_dispatch \
        [--experiment-glob 'configs/experiments/attn_pool_*_memmap.json'] \
        [--methods contrastive_logprob_recon_attn_pool_small,...] \
        [--seeds 0] \
        [--skip-existing]

Cell shape (consumed by scripts/dispatch/worker_attn_pool.sh):

    {
      "cell_id":           "seed_0__nq_memmap__contrastive_logprob_recon_attn_pool_small",
      "experiment_config": "configs/experiments/attn_pool_nq_memmap.json",
      "dataset":           "nq_memmap",
      "method":            "contrastive_logprob_recon_attn_pool_small",
      "seed":              0,
      "output_check":      "runs/attn_pool_nq_memmap/nq_memmap/contrastive_logprob_recon_attn_pool_small/seed_0/eval_metrics.json"
    }
"""
...
_DEFAULT_GLOB = "configs/experiments/attn_pool_*_memmap.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Populate dispatch pending/ queue for Issue #120 attn-pool sweep."
    )
