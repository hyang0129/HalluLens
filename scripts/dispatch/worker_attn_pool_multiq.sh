#!/usr/bin/env bash
# worker_attn_pool_multiq.sh — coordinator-free GPU worker for the Issue #121
# multi-query attn_pool sweep (k=2 + k=4, Llama + Qwen3, 6 datasets, seed 0).
#
# Dispatched via:
#   gpu_dispatch.py run [--force-concurrent] --node NODE bash scripts/dispatch/worker_attn_pool_multiq.sh
#
# Env vars (override defaults):
#   DISPATCH_ROOT — (default: shared/issue_121_dispatch)
...
DISPATCH_ROOT="${DISPATCH_ROOT:-$PROJECT_ROOT/shared/issue_121_dispatch}"
...
RUN_LOG="/tmp/issue_121_${WORKER_ID}.log"
...
_cleanup() {
  ...
  echo "worker_attn_pool_multiq $WORKER_ID exiting (trap)."
}
_signal_cleanup_and_exit() { _cleanup; exit 130; }
trap _cleanup EXIT
trap _signal_cleanup_and_exit INT TERM

echo "worker_attn_pool_multiq $WORKER_ID starting — dispatch_root=$DISPATCH_ROOT"
...
while true; do
  ...
  if [ -z "$CELL_PATH" ]; then
    echo "worker_attn_pool_multiq $WORKER_ID: no cells remaining, exiting."
    break
  fi
  ...
  if [ -f "$ABS_OUTPUT" ]; then
    echo "worker_attn_pool_multiq $WORKER_ID: output exists — marking done."
    "$PYTHON" "$CLI" complete --root "$DISPATCH_ROOT" --worker-id "$WORKER_ID" --cell "$CELL_PATH"
    N_SKIP=$(( N_SKIP + 1 ))
    continue
  fi

  echo "worker_attn_pool_multiq $WORKER_ID: running experiment=$EXPERIMENT method=$METHOD seed=$SEED"
  set +e
  ...
  if [ "$EXIT_CODE" -eq 0 ] && [ -f "$ABS_OUTPUT" ]; then
    "$PYTHON" "$CLI" complete --root "$DISPATCH_ROOT" --worker-id "$WORKER_ID" --cell "$CELL_PATH"
    N_DONE=$(( N_DONE + 1 ))
    echo "worker_attn_pool_multiq $WORKER_ID: completed $CELL_PATH"
    ...
    "$PYTHON" "$CLI" fail --root "$DISPATCH_ROOT" --worker-id "$WORKER_ID" --cell "$CELL_PATH" --err-file "$RUN_LOG"
    N_FAIL=$(( N_FAIL + 1 ))
    echo "worker_attn_pool_multiq $WORKER_ID: FAILED $CELL_PATH (exit $EXIT_CODE)"
  fi
done

kill "$HB_PID" 2>/dev/null
trap - EXIT
echo "worker_attn_pool_multiq $WORKER_ID processed $N_DONE done, $N_SKIP skipped, $N_FAIL failed."
