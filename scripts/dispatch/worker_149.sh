...
DISPATCH_ROOT="${DISPATCH_ROOT:-$PROJECT_ROOT/shared/issue_149_dispatch}"
...
RUN_LOG="/tmp/issue149_${WORKER_ID}.log"
...
_cleanup() {
  ...
  echo "worker_149 $WORKER_ID exiting (trap)."
}
_signal_cleanup_and_exit() { _cleanup; exit 130; }
trap _cleanup EXIT
trap _signal_cleanup_and_exit INT TERM

echo "worker_149 $WORKER_ID starting — dispatch_root=$DISPATCH_ROOT"
...
while true; do
  ...
  if [ -z "$CELL_PATH" ]; then
    echo "worker_149 $WORKER_ID: no cells remaining, exiting."
    break
  fi
  ...
  if [ -f "$ABS_OUTPUT" ]; then
    echo "worker_149 $WORKER_ID: output exists — marking done."
  ...
  if [ "$EXIT_CODE" -eq 0 ] && [ -f "$ABS_OUTPUT" ]; then
    "$PYTHON" "$CLI" complete --root "$DISPATCH_ROOT" --worker-id "$WORKER_ID" --cell "$CELL_PATH"
    N_DONE=$(( N_DONE + 1 ))
    echo "worker_149 $WORKER_ID: completed $CELL_PATH"
    ...
    "$PYTHON" "$CLI" fail --root "$DISPATCH_ROOT" --worker-id "$WORKER_ID" --cell "$CELL_PATH" --err-file "$RUN_LOG"
    N_FAIL=$(( N_FAIL + 1 ))
    echo "worker_149 $WORKER_ID: FAILED $CELL_PATH (exit $EXIT_CODE)"
  fi
done

kill "$HB_PID" 2>/dev/null
trap - EXIT
echo "worker_149 $WORKER_ID processed $N_DONE done, $N_SKIP skipped, $N_FAIL failed."
