...
"""
SciQ task for HalluLens.

Dataset: https://huggingface.co/datasets/allenai/sciq
  - train:      11,679 questions
  - validation:  1,000 questions
  - test:        1,000 questions  (total: 13,679)

Evaluation mode: CLOSED-BOOK (no supporting passage or distractors provided).
The model must rely on parametric memory; incorrect answers are treated as hallucinations.

Why SciQ?
  Science factual QA spanning physics, chemistry, biology, and earth science.
  Questions are typically single-hop but require domain-specific knowledge.
  This tests whether hallucination detection generalises to scientific domains
  beyond the encyclopaedic / multi-hop questions in TriviaQA / HotpotQA.
...
Output eval_results.json schema (ActivationParser-compatible):
  {
    "evaluator_abstantion": "sciq_no_abstain",
    "evaluator_hallucination": "sciq_substring_match",
    "abstantion": [false, ...],
...
def load_sciq_data(split="test", n_samples=None):
    """
    Load SciQ from HuggingFace (closed-book: no support passage or distractors).

    Args:
        split: "test" (1,000), "validation" (1,000), or "train" (11,679)
    ...
    print(f"Loading SciQ ({split}) from HuggingFace...")
    dataset = load_dataset("allenai/sciq", trust_remote_code=True)[split]

    data = []
    for idx, item in enumerate(dataset):
        data.append({
            "id": f"sciq_{split}_{idx}",
    ...
    print(f"Loaded {len(data)} SciQ samples from '{split}' split")
    return data
...
class SciQInference:
    """Run model inference on SciQ (closed-book, no context provided)."""
    ...
    def __init__(self, model_path, output_base_dir="output", split="test",
        ...
        self.task_output_dir = f"{output_base_dir}/sciq/{self.model_name}"
        ...
        self.data = load_sciq_data(split=split, n_samples=n_samples)
    ...
    def run_inference(self, inference_method="vllm", max_tokens=128, temperature=0.0,
        ...
        print(f"Starting SciQ inference | model={self.model_path} | split={self.split} | N={len(prompts_df)}")
    ...
    def run_inference_batched(self, batch_size=8, max_tokens=128, temperature=0.0,
        ...
        print(f"Starting batched SciQ inference | model={self.model_path} | "
...
class SciQEval:
    """Score model generations against SciQ gold answers."""
    ...
    def __init__(self, model_path, generations_file_path=None, output_base_dir="output",
        ...
        if generations_file_path:
            ...
            self.output_path = f"{output_base_dir}/sciq/{self.model_name}"
...
The question is a science factual question (physics, chemistry, biology, earth science, etc.).
...
class SciQLLMEval:
    """Score SciQ generations with an LLM judge, alongside substring match for comparison."""
    ...
    def run_eval(self, eval_results_path=None, resume=True):
        ...
        res = {
            "evaluator_abstantion": "sciq_no_abstain",
            "evaluator_hallucination": f"sciq_llm_judge:{self.evaluator}",
            "abstantion": [False] * n,
        ...
        print("\n" + "=" * 60)
        print("EVALUATION SUMMARY -- SciQ (LLM Judge vs Substring)")
        print("=" * 60)
        print(f"Model          : {self.model_name}")
...
def run_step(step, model, output_dir="output", split="test",
    ...
    """Run a single step of the SciQ task. Callable from Python directly.
    ...
    if step == "inference":
        ...
        raise ValueError(f"Unknown step '{step}'. SciQ supports: inference, eval, eval_llm")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SciQ inference and evaluation (closed-book)")
    ...
    parser.add_argument("--llm_evaluator", type=str, default=None,
                        help=f"Model to use as LLM judge (default: {SciQLLMEval.DEFAULT_EVALUATOR})")

    parser.add_argument("--model", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default="output")
    parser.add_argument("--split", type=str, default="test", choices=["test", "validation", "train"],
                        help="SciQ split to use (default: test = 1,000 questions)")
