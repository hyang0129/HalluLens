...
"""
SearchQA task for HalluLens.

Dataset: https://huggingface.co/datasets/kyunghyuncho/search_qa
  - train:       ~99,820 questions
  - validation:  ~13,893 questions
  - test:        ~27,247 questions

Evaluation mode: CLOSED-BOOK (search snippets are stripped — model relies on
parametric memory only).

Why SearchQA?
  Jeopardy-style factual QA with a distinctive answer distribution — many
  answers are proper nouns, dates, or short factoids.  The closed-book setting
  tests single-hop factual recall, complementing multi-hop (HotpotQA) and
  open-domain (TriviaQA / NQ) benchmarks.  Including SearchQA broadens the
  hallucination-detection evaluation to yet another question style and answer
  distribution.

Evaluation method:
  Correctness = case-insensitive substring match of the gold answer in the
  model response.
  Hallucination = response does not contain the gold answer.

Output eval_results.json schema (ActivationParser-compatible):
  {
    "evaluator_abstantion": "searchqa_no_abstain",
    "evaluator_hallucination": "searchqa_substring_match",
    "abstantion": [false, ...],
...
def load_searchqa_data(split="train", n_samples=None):
    """
    ...
    print(f"Loaded {len(data)} SearchQA samples from '{split}' split")
    return data
...
class SearchQAInference:
    """Run model inference on SearchQA (closed-book, no search snippets provided)."""

    def __init__(self, model_path, output_base_dir="output", split="train",
                 n_samples=None, generations_file_path=None):
        self.model_path = model_path
        self.model_name = model_path.split("/")[-1]
        self.split = split

        self.task_output_dir = f"{output_base_dir}/searchqa/{self.model_name}"
        ...
        self.data = load_searchqa_data(split=split, n_samples=n_samples)
    ...
    def run_inference(self, inference_method="vllm", max_tokens=128, temperature=0.0,
        ...
        print(f"Starting SearchQA inference | model={self.model_path} | split={self.split} | N={len(prompts_df)}")
    ...
    def run_inference_batched(self, batch_size=8, max_tokens=128, temperature=0.0,
        ...
        print(f"Starting batched SearchQA inference | model={self.model_path} | "
...
class SearchQAEval:
    """Score model generations against SearchQA gold answers."""
    ...
    def __init__(self, model_path, generations_file_path=None, output_base_dir="output",
        ...
        if generations_file_path:
            ...
            self.output_path = f"{output_base_dir}/searchqa/{self.model_name}"
    ...
    def run_eval(self, eval_results_path=None, log_file=None):
        ...
        print("\n" + "=" * 60)
        print("EVALUATION SUMMARY -- SearchQA")
        print("=" * 60)
...
This is a Jeopardy-style factual question. Accept semantically equivalent answers
even if phrased differently from the gold answer (e.g. different name orderings,
abbreviations, or minor spelling variations).
...
class SearchQALLMEval:
    """Score SearchQA generations with an LLM judge, alongside substring match for comparison."""
    ...
    def run_eval(self, eval_results_path=None, resume=True):
        ...
        res = {
            "evaluator_abstantion": "searchqa_no_abstain",
            "evaluator_hallucination": f"searchqa_llm_judge:{self.evaluator}",
            "abstantion": [False] * n,
        ...
        print("\n" + "=" * 60)
        print("EVALUATION SUMMARY -- SearchQA (LLM Judge vs Substring)")
        print("=" * 60)
        print(f"Model          : {self.model_name}")
...
def run_step(step, model, output_dir="output", split="train",
    ...
    """Run a single step of the SearchQA task. Callable from Python directly.
    ...
    if step == "inference":
        ...
        raise ValueError(f"Unknown step '{step}'. SearchQA supports: inference, eval, eval_llm")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SearchQA inference and evaluation (closed-book)")
    ...
    parser.add_argument("--llm_evaluator", type=str, default=None,
                        help=f"Model to use as LLM judge (default: {SearchQALLMEval.DEFAULT_EVALUATOR})")

    parser.add_argument("--model", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default="output")
    parser.add_argument("--split", type=str, default="train", choices=["train", "validation", "test"],
                        help="SearchQA split to use (default: train = ~99,820 questions)")
