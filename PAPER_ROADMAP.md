...
**Last updated:** 2026-05-18 (Loss-decomposition 2×2 ablation complete — issues #66 (`contrastive`, no recon) and #67 (`saplma_logprob_recon`) ran 40/40 cells across HotpotQA + PopQA × Llama-3.1-8B + Qwen3-8B × seeds [0,1,2,3,4]. **Headline finding: the supervised contrastive layer-pair InfoNCE objective carries essentially all the gain over SAPLMA; the logprob-recon auxiliary is decorative.** SAPLMA+recon vs SAPLMA is a wash in every cell (|Δ| ≤ 0.006). Contrastive alone vs SAPLMA: +0.020 (HotpotQA-Qwen3), +0.047 (PopQA-Qwen3), +0.132 (PopQA-Llama), +0.141 (HotpotQA-Llama). Recon on top of contrastive moves the needle only on PopQA-Qwen3 (+0.035); elsewhere flat or slightly negative. This invalidates the prior "logprob-recon is load-bearing" interpretation from the SimCLR-only result — that ablation showed self-supervised contrastive fails, but the new 2×2 attributes the win to *supervised* contrastive structure, not recon. §3 theory framing must pivot accordingly. See §4 item 7 for full numbers + the PopQA-Qwen3 interaction that §3 still needs to model.)
**Previously:** 2026-05-17 (LLM-judge ablation for ICR Probe explicitly cut — see #76. Verified the ICR paper's labeling protocol is undocumented (zero hits for "judge"/"evaluator"/"GPT" in paper text, no supplementary materials on ACL Anthology) and the released training code is broken (undefined `_load_data()`/`_create_data_loaders()` in `src/icr_probe.py`, public GitHub Issue #3 since Aug 2025 unresolved; third-party request for full pipeline GitHub Issue #5 Nov 2025 unanswered). We will not chase a number from an unreproducible paper; substring-match labeling stays primary, justified by literature standard (LLMsKnow, P(True), Farquhar et al.). Methodology footnote added to §4 item 10.)
...
| Ablation: SimCLR-only (unsupervised contrastive) | ✅ tried | Unsupervised SimCLR features + linear probe on top → ~random guessing (AUROC ≈ 0.5). Needs writeup; no further compute. Interpretation updated by the 2×2 below: self-supervised contrastive fails, *supervised* contrastive carries the signal — recon is not what made the difference. |
| Ablation: 2×2 loss decomposition (`contrastive` × `saplma_logprob_recon`) | ✅ done | 40/40 cells: 2 datasets (HotpotQA, PopQA) × 2 models (Llama-3.1-8B, Qwen3-8B) × 5 seeds. Verdict: supervised contrastive InfoNCE on layer pairs carries the win over SAPLMA (+0.02 to +0.14 AUROC across cells); logprob-recon as a free-floating auxiliary is decorative (SAPLMA+recon vs SAPLMA ≤ ±0.006 in every cell). Recon helps on top of contrastive in 1/4 cells (PopQA-Qwen3, +0.035). See §4 item 7 for the full table. Issues #66, #67, merged in PR #68. |
...
7. **Ablation: contrastive loss decomposition — 2×2 attribution complete (2026-05-18).** The full method's gain over SAPLMA decomposes into a contrastive-structure axis (BCE vs supervised layer-pair InfoNCE) and a recon-auxiliary axis (no recon vs +logprob-recon). The 2×2 ran across HotpotQA + PopQA × Llama-3.1-8B + Qwen3-8B × seeds [0,1,2,3,4] = 40 runs. Mean AUROC ± std:

   | Dataset / Model | SAPLMA | SAPLMA+recon | contrastive | contrastive+recon (full) |
   |---|---|---|---|---|
   | HotpotQA / Llama-3.1-8B | 0.6935 ± .003 | 0.6962 ± .002 | 0.8345 ± .005 | 0.8349 ± .006 |
   | HotpotQA / Qwen3-8B | 0.6986 ± .004 | 0.6992 ± .005 | 0.7186 ± .006 | 0.7199 ± .006 |
   | PopQA / Llama-3.1-8B | 0.6904 ± .003 | 0.6928 ± .004 | 0.8219 ± .006 | 0.8169 ± .009 |
   | PopQA / Qwen3-8B | 0.8063 ± .005 | 0.8005 ± .006 | 0.8532 ± .014 | 0.8881 ± .004 |

   Attribution (Δ AUROC relative to SAPLMA):

   | | recon alone | contrastive alone | recon on top of contrastive |
   |---|---|---|---|
   | HotpotQA-Llama | +0.003 | **+0.141** | +0.000 |
   | HotpotQA-Qwen3 | +0.001 | **+0.020** | +0.001 |
   | PopQA-Llama | +0.002 | **+0.132** | −0.005 |
   | PopQA-Qwen3 | −0.006 | **+0.047** | **+0.035** |

   **Verdict.** Supervised contrastive layer-pair InfoNCE is the load-bearing piece; logprob-recon as a free-floating auxiliary is decorative. SAPLMA+recon ≈ SAPLMA in every cell (|Δ| ≤ 0.006), so the recon target alone does not carry the hallucination signal. The headline gap over SAPLMA is attributable to the contrastive structure (+0.02 to +0.14 AUROC). Recon on top of contrastive is a wash or slightly negative in 3/4 cells; it helps meaningfully (+0.035) only on PopQA-Qwen3, where it also *hurts* SAPLMA — i.e. recon's value is contingent on the contrastive setup, not portable across architectures.

   **Paper implications.** (a) §3 theory framing should center the supervised contrastive layer-pair objective, not the recon auxiliary. The earlier "logprob-recon is load-bearing" reading (from the SimCLR-only ablation) was a confound — that ablation correctly showed self-supervised contrastive fails, but the load-bearing component is the *supervised label structure*, not recon. (b) The PopQA-Qwen3 recon×contrastive interaction is a real but narrow effect; either include it as a reported ablation (one line: "recon is decorative except for PopQA-Qwen3, where it interacts with the contrastive structure") or drop the recon term from the headline method and report it as an ablation row. Decide after §3 is drafted. (c) The SimCLR-only writeup (item still open) now reads as: "self-supervised contrastive fails because it learns invariances orthogonal to the hallucination axis; supervised contrastive on layer-pair views aligned to the hallucination label is what works." Issues #66, #67 closed by PR #68.
...
**Ablations** (HotpotQA + PopQA × Llama-3.1-8B + Qwen3-8B unless noted):
- SimCLR-only loss (Llama only; ✅ tried — random AUROC; writeup pending)
- 2×2 loss decomposition: `contrastive` (supervised InfoNCE, no recon) × `saplma_logprob_recon` (BCE + recon) — ✅ done (40/40 cells, see §4 item 7)
- Layer-pair sensitivity sweep (Llama only; coarse grid, not exhaustive — early/mid/late representatives)
