...
4. **Qwen seed sweep — 21 cells outstanding (as of 2026-05-15).** [`PAPER_ROADMAP.md`](../PAPER_ROADMAP.md) §2 reports 111/132 Qwen cells complete. If the 21 outstanding cells do not land by §5 freeze, the §4 model statement holds (we still ran Qwen) but §5 must hedge per-cell coverage. Re-run `python scripts/results_table.py` before §4 prose locks.
