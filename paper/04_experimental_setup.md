...
- **Training-pool cap (50k).** The per-cell training pool — train + validation, before each seed's random split — is capped at the first 50,000 cached samples per (model, dataset); datasets whose labeled pool is smaller are used in full. The cap binds only on the largest datasets (HotpotQA, MMLU), for which the cached `_0-50000` slice is the training source and the effective post-split train size is ≈45k after the 10% validation hold-out. The held-out test split is a separate cache and is unaffected by the cap. Exact per-dataset post-split train/val sizes live in Appendix D (sourced via `\result` macros); §4 states only the cap structurally.
