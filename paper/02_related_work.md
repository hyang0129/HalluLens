...
**Bib-policy reminder.** Bibkeys in the "References — materialized for §2" section below are already in `paper/references.bib`. Any inline cite marked `[PENDING-APPROVAL]` in the bullets below, plus everything in the "Pending-approval candidates" section at the very bottom, is **not** yet in `.bib` and must be human-approved before insertion (per `paper/references.bib` policy header and `CLAUDE.md`). As of 2026-05-19: `zhang2025icr` (ICR Probe) and `barshalom2025actvit` (ACT-ViT) are pending-approval but are live baselines in the §4 grid, so they will need approval before the writing agent can lock §2.1 prose.
...
- **CLAP — contemporaneous *cross-layer-attention* probe.** [PENDING-APPROVAL: full candidate entry below — Liu et al., arXiv:2509.09700, Sep 2025. Surfaced by the 2026-05-19 literature pass; not currently in our baseline grid. Add to `.bib` only after human approval.] Technical content the prose should land:
...
- **(REQUIRED — live baseline in the grid)** Zhang et al., "ICR Probe: Tracking Hidden State Dynamics for Reliable Hallucination Detection in LLMs," ACL 2025, arXiv:2507.16488. Proposed bibkey: `zhang2025icr`.
  ...
  - *Verify before adding to `.bib`.* (a) Correct ACL Anthology / arXiv identifier and final title; (b) that the methodology footnote we plan in §4 item 10 (irreproducibility — `output_judge.jsonl` not released, training code references undefined `_load_data()`, GitHub Issues #3 and #5 unanswered) is accurate as written — re-check the repo and ACL Anthology supplementary listing before submission; (c) confirm "ACL 2025" venue vs. "ACL Findings 2025" before citing.

- **(REQUIRED for §2.1 — surfaced by 2026-05-19 literature pass; NOT in our baseline grid)** Liu et al., "Cross-Layer Attention Probing for Fine-Grained Hallucination Detection" (CLAP), arXiv:2509.09700, Sep 2025. Proposed bibkey: `liu2025clap` (verify author list and surnames before approval).
  ...
  - *Verify before adding to `.bib`.* (a) Confirm full author list (the agent's search returned "Liu et al." only); (b) confirm arXiv ID 2509.09700 resolves to the right paper title (URL: https://arxiv.org/abs/2509.09700); (c) confirm venue status (arXiv-only? in submission?).
...
- **(REQUIRED — live baseline in the grid)** Bar-Shalom et al., "ACT-ViT: A Vision Transformer Over Full LLM Activation Tensors for Hallucination Detection," arXiv:2510.00296. Proposed bibkey: `barshalom2025actvit` (verify author surname spelling).
  ...
  - *Verify before adding to `.bib`.* (a) Confirm full author list and surname spelling (`act_vit.py` only credits "Bar-Shalom et al." in the docstring); (b) confirm venue status (arXiv-only as of 2026-05-19? in submission? accepted somewhere?) — affects how we cite it; (c) confirm arXiv ID 2510.00296 resolves to the right paper title.
